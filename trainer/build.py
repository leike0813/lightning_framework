...
def build_trainer(train_node=DEFAULT_CONFIG, model_node=EXAMPLE_MODEL_CONFIG, env_node=DEFAULT_ENVIRONMENT, extra_callbacks=[]):
    ...
    if train_node.LOGGER.NAME.lower() == 'mlflowlogger':
        logger = MLFlowLogger(
            ...
            run_name=run_name,
            checkpoint_path_prefix=train_node.LOGGER.MLFLOW.checkpoint_path_prefix, # incompatible with lightning 2.2
            tags=train_node.LOGGER.TAGS,
            log_model=True,
        )
    elif train_node.LOGGER.NAME.lower() == 'csvlogger':
        logger = CSVLogger(
            save_dir=os.path.join(env_node.RESULT_BASE_PATH, train_node.LOGGER.CSVLOGGER.save_dir),
            name=train_node.LOGGER.CSVLOGGER.name,
            version=train_node.LOGGER.CSVLOGGER.version,
            prefix=train_node.LOGGER.CSVLOGGER.prefix,
            flush_logs_every_n_steps=train_node.LOGGER.CSVLOGGER.flush_logs_every_n_steps,
        )
    else:
        logger = None
    # build callbacks
