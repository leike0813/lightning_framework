...
from cv_utils import torch_merge_alpha, torch_align_channels
...
def build_image_prediction_writer(node=DEFAULT_CONFIG):
    return ImagePredictionWriter(**node)
...
class ImagePredictionWriter(BasePredictionWriter):
    ...
    def __init__(self, output_dir, image_format='auto', concatenate=False, log_prediction=False, log_folder='', write_interval='batch'):
        super(ImagePredictionWriter, self).__init__(write_interval)
        ...
        assert image_format.lower() in ImagePredictionWriter.AVAILABLE_FORMATS, "Invalid image format. The available formats are 'jpg', 'jpeg', 'png', 'bmp', 'gif' and 'auto'"
    ...
    def write_prediction(pred, img_names, output_dir, concatenate, log_prediction,
        ...
        if not concatenate:
            for suffix, suffix_imgs in pred.items():
                ...
                if not suffix.startswith('.'):
                    if image_format.lower() == 'auto':
                        suffix, fmt, mode = ImagePredictionWriter.infer_format_and_mode(suffix, suffix_imgs)
                    else:
                        suffix, _ = ImagePredictionWriter.infer_format(suffix)
                        fmt = image_format.lower() if image_format.lower() in ImagePredictionWriter.AVAILABLE_FORMATS else 'png'
                        mode, suffix_imgs = ImagePredictionWriter.infer_mode(suffix_imgs, fmt)
            ...
            for suffix, suffix_imgs in pred.items():
                ...
                assert suffix_imgs.shape[0] == len(img_names), 'Unmatched number of image names.'
                if not suffix.startswith('.'):
                    suffix, _fmt, _mode = ImagePredictionWriter.infer_format_and_mode(suffix, suffix_imgs)
                    if _fmt in ['png', 'gif']:
    ...
    def infer_format_and_mode(name, img_tensor):
        ...
        name_main, name_fmt = ImagePredictionWriter.infer_format(name)
...
if __name__ == '__main__':
    ...
    writer = ImagePredictionWriter(output_dir)
